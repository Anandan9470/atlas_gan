...
class VoxelGrid(Structure):
    ...
    def __init__(self, *, cloud, n_xyz=None, size_xyz=None,
                 segments=None, regular_bounding_box=True):
        ...
        if segments != None:
            self.x_y_z = [len(segments[0])-1, len(segments[1])-1, len(segments[2])-1]
            if self.x_y_z[0] == 0:
                self.x_y_z[0] = 1
            if self.x_y_z[1] == 0:
                self.x_y_z[1] = 1
    ...
    def extract_info(self):
        ...
        self.energies = self.cloud.energies
    ...
    def get_feature_vector(self, mode="binary"):
        ...
        elif mode == 'mean_by_energy':

            data_dict = {'indices': self.voxel_n, 'energies': self.energies}
            energy_df = pd.DataFrame(data=data_dict)

            energy_df = energy_df.groupby('indices')['energies'].sum()
            indices = energy_df.index
            energies = energy_df.values
            vector[indices] = energies

            return vector.reshape(self.x_y_z)

    def query_voxels(self, points, energies):

        voxel_n = self.query(points)
        vector = np.zeros(self.n_voxels)

        data_dict = {'indices': voxel_n, 'energies': energies}
        energy_df = pd.DataFrame(data=data_dict)
